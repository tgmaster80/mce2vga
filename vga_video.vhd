...
--		vsync_level 			:= '0'
	...
	generic(
		...
		constant scale_mode				: integer   := 0  -- 0 : no scale, 1 : CGA doubling rows, 2 : MDA 350->400, 3 : double rows and columns
	);
...
begin
					...
					case scale_mode is
						when 3 =>
							if (col_repeat_phase = '0') then
								next_compact_accum := col_compact_accum + 78;
	...
	begin
		if (rising_edge(clk)) then
			videov <= '0'; 
			if ( vcount > 2 and vcount < vert_active_video + 1 and row_number < max_row) then 
				videov <= '1';
			end if;	
	   end if;
	end process;
